...
Scene::Scene(const std::string& work_dir, const std::string& scene_file_name) {
    ...
    // load light sources
    int light_sources_size = scene_data["light_source"].size();
    for (int i = 0; i < light_sources_size; i++) {
        auto pos_vec = toml::find<std::vector<float>>(scene_data["light_source"][i], "position");
        Point position(pos_vec[0], pos_vec[1], pos_vec[2]);
        float intensity = toml::find<float>(scene_data["light_source"][i], "intensity");
        light_sources_.emplace_back(position, intensity);
    }
    ...
    for (int i = 0; i < hittable_size; i++) {
        // load material
        auto ambient_vec =  toml::find<std::vector<float>>(scene_data["hittable"][i], "ambient");
        ...
        Color ambient(ambient_vec[0], ambient_vec[1], ambient_vec[2]);
        Color diffuse(diffuse_vec[0], diffuse_vec[1], diffuse_vec[2]);
        Color specular(specular_vec[0], specular_vec[1], specular_vec[2]);

        float k_a = toml::find<float>(scene_data["hittable"][i], "k_a");
        float k_d = toml::find<float>(scene_data["hittable"][i], "k_d");
        float k_s = toml::find<float>(scene_data["hittable"][i], "k_s");
        float sh = toml::find<float>(scene_data["hittable"][i], "sh");

        material_pool_.emplace_back(new Material(ambient, diffuse, specular, k_a, k_d, k_s, sh));
